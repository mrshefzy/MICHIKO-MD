...
async function getPrayerTimes(city) {
    ...
    const url = `https://jadwal-sholat.tirto.id/kota-${formattedCity}`;

    try {
        const response = await fetch(url);
        
        if (!response.ok) {
            throw new Error(`Failed to fetch prayer times for ${city}. Status: ${response.status}`);
        }
        ...
        const todayRow = $('table tr').eq(1);
        const prayerTimes = {
            subuh: todayRow.find('td').eq(1).text().trim(),
            duha: todayRow.find('td').eq(2).text().trim(),
            dzuhur: todayRow.find('td').eq(3).text().trim(),
            ashar: todayRow.find('td').eq(4).text().trim(),
            maghrib: todayRow.find('td').eq(5).text().trim(),
            isya: todayRow.find('td').eq(6).text().trim(),
        };

        Object.keys(prayerTimes).forEach(key => {
            if (!prayerTimes[key]) {
                prayerTimes[key] = '(tidak ditemukan)';
            }
        });

        const result = `*JADWAL SHALAT KOTA ${city.toUpperCase()}*\n` +
            `📅 ${formattedDate}\n\n` +
            `🌄 *Subuh*: ${prayerTimes.subuh}\n` +
            `🌅 *Duha*: ${prayerTimes.duha}\n` +
            `☀️ *Dzuhur*: ${prayerTimes.dzuhur}\n` +
...
module.exports = { pinterest, pinterest2, remini, mediafire, tiktokDl, getPrayerTimes }